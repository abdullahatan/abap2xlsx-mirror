CLASS zcl_facex_theme_eclrschemelst DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_facex_theme_eclrschemelst IMPLEMENTATION.
  ...
  METHOD build_xml.
    ...
    IF extracolor IS INITIAL.
      lo_theme_objdef ?= io_document->create_simple_element_ns( prefix = zcl_facex_theme=>c_theme_prefix
                                                                name   = zcl_facex_theme=>c_theme_extra_color
                                                                parent = lo_theme ).
  ...
  METHOD load.
    extracolor = zcl_facex_common=>clone_ixml_with_namespaces( io_extra_color ).
  ENDMETHOD.                    "load
ENDCLASS.
