CLASS zcl_facex_row DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_facex_row IMPLEMENTATION.
  ...
  METHOD get_collapsed.
    ...
    DATA: lt_row_outlines  TYPE zcl_facex_worksheet=>mty_ts_outlines_row,
...
* Maybe we can use the outline information in the worksheet
  ...
  METHOD set_outline_level.
    IF   ip_outline_level < 0
      ...
      zcx_facex=>raise_text( 'Outline level must range between 0 and 7.' ).
  ...
  METHOD set_row_height.
    ...
    TRY.
        ...
        IF height <= 0.
          zcx_facex=>raise_text( 'Please supply a positive number as row-height' ).
        ENDIF.
        me->row_height = ip_row_height.
      CATCH cx_sy_conversion_no_number.
        zcx_facex=>raise_text( 'Unable to interpret ip_row_height as number' ).
    ENDTRY.
