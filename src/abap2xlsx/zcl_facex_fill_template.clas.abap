CLASS zcl_facex_fill_template DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: tv_range_name TYPE c LENGTH 30,
           BEGIN OF ts_range,
             sheet  TYPE zfacex_sheet_title,
             name   TYPE tv_range_name,
             start  TYPE zfacex_cell_row,
             stop   TYPE zfacex_cell_row,
             id     TYPE i,
             parent TYPE i,
             length TYPE i,
           END OF ts_range,
           ...
           tt_sheet_titles TYPE STANDARD TABLE OF zfacex_sheet_title WITH DEFAULT KEY,
    ...
    DATA mo_excel TYPE REF TO zcl_facex READ-ONLY.
    ...
    CLASS-METHODS create
      ...
      RETURNING
        VALUE(eo_template_filler) TYPE REF TO zcl_facex_fill_template
      RAISING
        zcx_facex.

    METHODS fill_sheet
      IMPORTING
        !iv_data TYPE zcl_facex_template_data=>ts_template_data_sheet
      RAISING
        zcx_facex .
  ...
  PRIVATE SECTION.
    ...
    TYPES: tt_cell_data_no_key TYPE STANDARD TABLE OF zfacex_s_cell_data WITH DEFAULT KEY.

    METHODS fill_range
      IMPORTING
        !iv_sheet              TYPE zfacex_sheet_title
        !iv_parent             TYPE zfacex_cell_row
        !iv_data               TYPE data
        VALUE(iv_range_length) TYPE zfacex_cell_row
        !io_sheet              TYPE REF TO zcl_facex_worksheet
      CHANGING
        !ct_cells              TYPE tt_cell_data_no_key
        !cv_diff               TYPE zfacex_cell_row
        !ct_merged_cells       TYPE zcl_facex_worksheet=>mty_ts_merge
      RAISING
        zcx_facex .
    METHODS get_range .
    METHODS validate_range
      IMPORTING
        !io_range TYPE REF TO zcl_facex_range .
...
CLASS zcl_facex_fill_template IMPLEMENTATION.
  ...
  METHOD fill_range.
    ...
    DATA: lt_tmp_cells_template        TYPE tt_cell_data_no_key,
          ...
          ls_cell                      TYPE zfacex_s_cell_data,
          lt_tmp_merged_cells_template TYPE zcl_facex_worksheet=>mty_ts_merge,
          lt_merged_cells_result       TYPE zcl_facex_worksheet=>mty_ts_merge,
          lt_tmp_merged_cells          TYPE zcl_facex_worksheet=>mty_ts_merge,
...
*      update cells before any range
      ...
      LOOP AT ct_cells INTO ls_cell  WHERE cell_row >= lv_start_row AND cell_row <= lv_stop_row .
        ...
        lv_column_alpha = zcl_facex_common=>convert_column2alpha( ls_cell-cell_column ).
...
*      replace variables of current range with data
  ...
  METHOD fill_sheet.
    ...
    DATA: lo_worksheet      TYPE REF TO zcl_facex_worksheet,
          ...
          lt_merged_cells   TYPE zcl_facex_worksheet=>mty_ts_merge,
          lt_merged_cells_2 TYPE zcl_facex_worksheet=>mty_ts_merge,
          lv_initial_diff   TYPE i.
    FIELD-SYMBOLS:
      <any_data>       TYPE any,
      <ls_sheet_cell>  TYPE zfacex_s_cell_data,
      <ls_merged_cell> TYPE zcl_facex_worksheet=>mty_merge.
  ...
  METHOD find_var.
    ...
    DATA: lv_row            TYPE i,
          ...
          lv_highest_column TYPE zfacex_cell_column,
    ...
    LOOP AT mt_sheet ASSIGNING <lv_sheet>.
      ...
      WHILE lv_row <= lv_highest_row.
        ...
        WHILE lv_column <= lv_highest_column.
          lv_column_alpha = zcl_facex_common=>convert_column2alpha( lv_column ).
          CLEAR lo_style.
  ...
  METHOD get_range.
    ...
    DATA:
      lo_worksheets_iterator TYPE REF TO zcl_facex_collection_iterator,
      lo_worksheet           TYPE REF TO zcl_facex_worksheet,
      lo_range_iterator      TYPE REF TO zcl_facex_collection_iterator,
