CLASS zcl_facex_legacy_palette DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
*"* do not include other source files here!!!
    ...
    METHODS get_color
      IMPORTING
        !ip_index       TYPE i
      RETURNING
        VALUE(ep_color) TYPE zfacex_style_color_argb
      RAISING
        zcx_facex .
    METHODS get_colors
      RETURNING
        VALUE(ep_colors) TYPE zfacex_t_style_color_argb .
    METHODS set_color
      IMPORTING
        !ip_index TYPE i
        !ip_color TYPE zfacex_style_color_argb
      RAISING
        zcx_facex .
  PROTECTED SECTION.
...
*"* do not include other source files here!!!
    ...
    DATA colors TYPE zfacex_t_style_color_argb .
ENDCLASS.



CLASS zcl_facex_legacy_palette IMPLEMENTATION.
  ...
  METHOD get_color.
    ...
    IF sy-subrc <> 0.
      zcx_facex=>raise_text( 'Invalid color index' ).
    ENDIF.
  ENDMETHOD.
